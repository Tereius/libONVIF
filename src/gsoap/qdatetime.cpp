...
#endif
#if defined(QT_SUPPORTS)
#if QT_SUPPORTS(timezone)
#define HAS_QT_TIMEZONE
#endif
#elif !defined(QT_NO_TIMEZONE)
#define HAS_QT_TIMEZONE
#endif
#if(QT_VERSION >= QT_VERSION_CHECK(5, 2, 0)) && defined(HAS_QT_TIMEZONE)
#include <QTimeZone>
#endif
...
QDateTime *soap_in_xsd__dateTime(struct soap *soap, char const *tag, QDateTime *a, char const *type) {
	if(soap_element_begin_in(soap, tag, 0, NULL)) return NULL;
	if(*soap->type && soap_match_att(soap, soap->type, type) && soap_match_att(soap, soap->type, ":dateTime")) {
	...
	if(*soap->href == '#') {
		...
		if(soap_s2xsd__dateTime(soap, soap_value(soap), a)) return NULL;
	}
	...
	if(s) {
		...
		if(*t) {
			...
			if(*t == '+' || *t == '-') {
				...
				if(*t) return soap->error = SOAP_TYPE;
...
static void *instantiate_xsd__dateTime(struct soap *soap, int n, const char *type, const char *arrayType, size_t *size) {
	...
	if(n < 0) {
		...
		if(size) *size = sizeof(QDateTime);
	} else {
		cp->ptr = SOAP_NEW_ARRAY(soap, QDateTime, n);
		if(size) *size = n * sizeof(QDateTime);
	}
	DBGLOG(TEST, SOAP_MESSAGE(fdebug, "Instantiated location=%p\n", cp->ptr));
	if(!cp->ptr) soap->error = SOAP_EOM;
